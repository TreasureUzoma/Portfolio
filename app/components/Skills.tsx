        ...
        <div className="my_fixed_width">
          ...
          <div className="grid place-items-center my-5 gap-5 sm:grid-cols-2 md:grid-cols-3">
            {skillsList.map((skill, index) => (
              <div
                key={skill.name}
                className="w-full border-[rgba(255,255,255,0.1)] rounded-xl flex items-center relative overflow-hidden p-4 bg-[#0a1c20] font-clash transition-all duration-300 text-[0.9rem] hover:scale-95 md:hover:scale-105 border-2"
              >
                <figure className="rounded-lg bg-[rgba(255,255,255,0.1)] w-10 h-10 mr-3 flex justify-center items-center">
                  <Image
                    src={skill.image}
                    alt={skill.name}
                    width={29}
